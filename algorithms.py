...
class CRC(Algorithm):
    ...
    def __call__(self) -> bool:
        ...
        key = input("Key: ")
        arr_key = np.array([int(char) for char in key])
        return self.remainder_receiver(self.data.content, arr_key)

    def remainder_receiver(self, data, key):

        remainder_CRC = self.mod2div(data, key)
...
class Checksum(Algorithm):
    ...
    def prepare(self) -> None:
        ...
        array_with_checksum = np.reshape(
            array_with_checksum, (data_as_arr.shape[0] + 1, 16)
        )

        self.data.content = array_with_checksum.flatten()
    ...
    def _call_(self) -> bool:
        ...
        digit_sum_and_overflow_receiveddata = self.sumsubstrings_and_overflow(
            data_as_arr
        )
        summed_substrings_receiveddata = self.sumsubstrings_and_overflow(
            digit_sum_and_overflow_receiveddata
        )
        # in case there's new overflow from adding the previous overflow to the previous summed substrings
        while 1 in summed_substrings_receiveddata[1::]:
            summed_substrings_receiveddata = self.sumsubstrings_and_overflow(
                summed_substrings_receiveddata
            )

        complement_checksum_and_substrings = self.the_ones_complement(
            summed_substrings_receiveddata
        )
